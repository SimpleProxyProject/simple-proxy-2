...
def root(url: str, request: Request, response: Response, api_key: APIKey = Depends(get_api_key)):
    ...
    try:
        ...
        mobile_useragents = [
            ...
            'Mozilla/5.0 (Linux; Android 11; SM-A205U) AppleWebKit/537.36 '
            '(KHTML, like Gecko) Chrome/91.0.4472.77 Mobile Safari/537.36',
            'Mozilla/5.0 (Linux; Android 11; SM-A102U) AppleWebKit/537.36'
            ' (KHTML, like Gecko) Chrome/91.0.4472.77 Mobile Safari/537.36',
        ...
        resp = requests.get(url, proxies=proxies, headers=headers, cookies=cookies, timeout=3)
...
def get_ip():
    try:
        resp = requests.get('http://ip-api.com/json', proxies={'http': PROXY_PATH})
